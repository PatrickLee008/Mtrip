#!/usr/bin/env bash
# Runs stage 4 final approval and credential delivery tests in a disposable database.
...
TEST_DB="mtrip_onboarding_final_approval_test"
MIGRATION="$REPO_ROOT/database/migrations/V20260915210000__add-onboarding-final-approval.sql"
...
sed -e "s/USE \`mtrip_business\`;/USE \`$TEST_DB\`;/" -e "s/USE \`mtrip_system\`;/USE \`$TEST_DB\`;/" "$MIGRATION" | mysql_exec
sed -e "s/USE \`mtrip_business\`;/USE \`$TEST_DB\`;/" -e "s/USE \`mtrip_system\`;/USE \`$TEST_DB\`;/" "$MIGRATION" | mysql_exec
...
$DOCKER cp "$REPO_ROOT/backend/services/merchant-service/test/onboarding-final-approval.php" "$SERVICE_CONTAINER:/tmp/onboarding-final-approval.php"
$DOCKER cp "$REPO_ROOT/backend/services/merchant-service/test/onboarding-final-approval-worker.php" "$SERVICE_CONTAINER:/tmp/onboarding-final-approval-worker.php"
$DOCKER exec \
  -e DB_BUSINESS_DATABASE="$TEST_DB" \
  -e DB_SYSTEM_DATABASE="$TEST_DB" \
  "$SERVICE_CONTAINER" php -d display_errors=1 /tmp/onboarding-final-approval.php

echo "Stage 4 onboarding final approval tests passed"
