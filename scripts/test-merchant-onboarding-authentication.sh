#!/usr/bin/env bash
# Runs stage 5 activation and login tests in a disposable database.
...
TEST_DB="mtrip_onboarding_authentication_test"
MIGRATION="$REPO_ROOT/database/migrations/V20260916000000__add-merchant-activation-login.sql"
...
sed -e "s/USE \`mtrip_business\`;/USE \`$TEST_DB\`;/" "$MIGRATION" | mysql_exec
sed -e "s/USE \`mtrip_business\`;/USE \`$TEST_DB\`;/" "$MIGRATION" | mysql_exec
...
$DOCKER cp "$REPO_ROOT/backend/services/merchant-service/test/onboarding-authentication.php" "$SERVICE_CONTAINER:/tmp/onboarding-authentication.php"
$DOCKER exec \
  -e DB_BUSINESS_DATABASE="$TEST_DB" \
  -e DB_SYSTEM_DATABASE="$TEST_DB" \
  "$SERVICE_CONTAINER" php -d display_errors=1 /tmp/onboarding-authentication.php

echo "Stage 5 merchant activation and login tests passed"
