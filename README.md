...
client-app 邀请码接后端(2026-09-21):Refer & Earn / Referral Status 两页与共用统计卡此前整页读设计稿假数据,现接 `user-service` `/api/v1/app/user/referral/my|invitees`(`my` 增 `pendingCount`/`rewardedCount`,`invitees` 增选填 `status` 供两个页签过滤),取数抽进 `screens/more/useReferralData.ts` 两页共用。注册绑定链路本就存在、未改。**发放时机按 PRD 口径纠正**:原为支付成功即发,现移到 `BookingLifecycleService::checkIn`(商户后台点入住 / 商户核销 / 平台手工核销三个入口共用),已真机跑通并验证幂等,测试数据已还原;⚠️ 撤销入住不退回已发奖励。同时补迁移 `V20260921030000` 种下 `referral_reward_inviter/invitee=50000` —— 此前全库零行,导致首单达成时钱包一分不进而 `reward_status` 已置 1;⚠️ 仅写 MMK 站点(5/6/7),50000 落到 EUR 站点就是每单 5 万欧元,EUR 站点须按欧元口径另配。⚠️ 奖励为 0 时仍会置 `reward_status=1`(按用户决定保留现状)。验收:backend `php -l` 393/393、shared 单测 99 例全过、admin-web build、client-app typecheck 均通过;`scripts/check.ps1` 本机仍不可跑(未装 php),两步 PHP 检查在容器内完成。
