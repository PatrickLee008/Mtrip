...
CREATE TABLE IF NOT EXISTS `user_info` (
  ...
  `real_name_status`   TINYINT      NOT NULL DEFAULT 0 COMMENT '实名认证状态:0未认证 1已认证 2认证失败 3审核中',
  `real_name`          VARCHAR(255) NOT NULL DEFAULT '' COMMENT '真实姓名(加密)',
  `id_card`            VARCHAR(255) NOT NULL DEFAULT '' COMMENT '身份证号(加密)',
  `gender`             TINYINT      NOT NULL DEFAULT 0 COMMENT '性别:0未填 1男 2女 3其他',
  `birthday`           DATE         NULL DEFAULT NULL COMMENT '出生日期',
  `city`               VARCHAR(100) NOT NULL DEFAULT '' COMMENT '常住城市',
  `home_address`       VARCHAR(1024) NOT NULL DEFAULT '' COMMENT '家庭住址(加密)',
  `profile_setup_at`   DATETIME     NULL DEFAULT NULL COMMENT '资料向导第1步完成时间(空=未完善,App 据此弹窗)',
  `nationality`        VARCHAR(10)  NOT NULL DEFAULT '' COMMENT '国籍(ISO 3166-1 alpha-2)',
  `id_card_front`      VARCHAR(255) NOT NULL DEFAULT '' COMMENT '证件正面图URL',
  `id_card_back`       VARCHAR(255) NOT NULL DEFAULT '' COMMENT '证件背面图URL',
  `selfie_image`       VARCHAR(255) NOT NULL DEFAULT '' COMMENT '自拍图URL',
  `real_name_submit_at` DATETIME    NULL DEFAULT NULL COMMENT '实名资料提交时间',
  `real_name_audit_by` VARCHAR(64)  NOT NULL DEFAULT '' COMMENT '实名审核人',
  `real_name_audit_at` DATETIME     NULL DEFAULT NULL COMMENT '实名审核时间',
  `real_name_reject_reason` VARCHAR(500) NOT NULL DEFAULT '' COMMENT '实名驳回原因',
