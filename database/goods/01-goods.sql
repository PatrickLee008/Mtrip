...
CREATE TABLE IF NOT EXISTS `hotel_room_type` (
  ...
  `bed_count`    TINYINT      NOT NULL DEFAULT 1 COMMENT '床数量',
  `area`         VARCHAR(20)  NOT NULL DEFAULT '' COMMENT '面积(㎡)',
  `max_adults`   TINYINT      NOT NULL DEFAULT 2 COMMENT '最多成人数',
  `max_children` TINYINT      NOT NULL DEFAULT 0 COMMENT '最多儿童数',
  `max_guests`   TINYINT      NOT NULL DEFAULT 2 COMMENT '最大入住人数',
  `floor_name`   VARCHAR(50)  NOT NULL DEFAULT '' COMMENT '楼层范围',
  `room_view`    VARCHAR(80)  NOT NULL DEFAULT '' COMMENT '景观',
  `smoking`      TINYINT      NOT NULL DEFAULT 0 COMMENT '是否允许吸烟:0否 1是',
  ...
  `extra_bed_price` DECIMAL(12,2) NOT NULL DEFAULT 0.00 COMMENT '加床价',
  `base_stock`   INT          NOT NULL DEFAULT 0 COMMENT '基础每日库存',
  `launch_stock` INT          NOT NULL DEFAULT 0 COMMENT '发布首日可售库存',
...
CREATE TABLE IF NOT EXISTS `goods_daily_stock` (
  ...
  `min_stay`     INT          NOT NULL DEFAULT 1 COMMENT '最少入住晚数',
  `max_stay`     INT          NOT NULL DEFAULT 30 COMMENT '最多入住晚数',
  `closed_to_arrival` TINYINT NOT NULL DEFAULT 0 COMMENT 'CTA:是否关闭入住',
  `closed_to_departure` TINYINT NOT NULL DEFAULT 0 COMMENT 'CTD:是否关闭离店',
  `source`       VARCHAR(30)  NOT NULL DEFAULT 'manual' COMMENT '库存来源:manual/pms/channel/base',
  `note`         VARCHAR(255) NOT NULL DEFAULT '' COMMENT '商户备注',
  `created_at`  DATETIME     NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `updated_at`  DATETIME     NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
  `deleted_at`  DATETIME     NULL DEFAULT NULL COMMENT '删除时间(软删)',
  PRIMARY KEY (`id`),
