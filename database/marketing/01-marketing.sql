...
CREATE TABLE IF NOT EXISTS `marketing_coupon` (
  ...
  `description`    VARCHAR(255) NOT NULL DEFAULT '' COMMENT '卡面描述(稿面卡片正文)',
  `coupon_type`    TINYINT      NOT NULL DEFAULT 1 COMMENT '券类型:1满减券 2折扣券 3无门槛券',
  `promotion_kind` TINYINT      NOT NULL DEFAULT 0 COMMENT '促销形态:0未分类(存量/平台券) 1百分比 2固定金额 3优惠码 4长住',
  `promo_code`     VARCHAR(32)  NOT NULL DEFAULT '' COMMENT '优惠码(promotion_kind=3 必填,同一站点内唯一)',
  ...
  `min_nights`     INT          NOT NULL DEFAULT 0 COMMENT '长住促销最低入住晚数(promotion_kind=4 必填,0=不限)',
  `max_nights`     INT          NOT NULL DEFAULT 0 COMMENT '适用入住时长上限(0=不限)',
  `book_advance_days` INT       NOT NULL DEFAULT 0 COMMENT '提前预订优惠:需提前 N 天预订(0=不限制)',
  ...
  `remark`         VARCHAR(500) NOT NULL DEFAULT '' COMMENT '备注/使用说明(面向客人的条款)',
  `staff_note`     VARCHAR(500) NOT NULL DEFAULT '' COMMENT '内部员工备注(稿面 Internal Staff Notes,不对客人展示)',
  ...
  KEY `idx_site_promo_code` (`site_id`, `promo_code`),
