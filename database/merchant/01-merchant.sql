...
CREATE TABLE IF NOT EXISTS `merchant_info` (
  ...
  `suspended_until`     DATETIME NULL COMMENT 'UTC暂停截止时间',
  `active_suspension_id` BIGINT UNSIGNED NULL,
  `reactivation_requires_super` TINYINT NOT NULL DEFAULT 0,
...
CREATE TABLE IF NOT EXISTS `merchant_admin` (
  ...
  `mobile_index`  CHAR(64)     NULL DEFAULT NULL COMMENT '登录账号手机检索哈希',
  `email`         VARCHAR(255) NOT NULL DEFAULT '' COMMENT '登录账号邮箱(加密)',
  `email_index`   CHAR(64)     NULL DEFAULT NULL COMMENT '登录账号邮箱检索哈希',
  `verified_mobile_at` DATETIME NULL COMMENT '最近一次验证注册手机号时间',
  `verified_email_at` DATETIME NULL COMMENT '最近一次验证注册邮箱时间',
  `google_subject_hash` CHAR(64) NULL COMMENT '已关联Google subject检索哈希',
  `google_email` VARCHAR(255) NOT NULL DEFAULT '' COMMENT '已关联Google邮箱(加密)',
  `google_linked_at` DATETIME NULL COMMENT 'Google身份关联时间',
  `activated_at` DATETIME NULL COMMENT '首次账号激活时间',
  `last_login_method` VARCHAR(30) NOT NULL DEFAULT '' COMMENT '最近认证方式',
  ...
  UNIQUE KEY `uk_google_subject_hash` (`google_subject_hash`),
  KEY `idx_site_id` (`site_id`),
  KEY `idx_mobile_index` (`site_id`,`mobile_index`),
  KEY `idx_email_index` (`site_id`,`email_index`),
  KEY `idx_merchant_id` (`merchant_id`),
  KEY `idx_group_id` (`group_id`),
