...
CREATE TABLE IF NOT EXISTS `merchant_store` (
  ...
  `contact_phone2`   VARCHAR(255) NOT NULL DEFAULT '' COMMENT '物业备用联系电话(加密)',
  `contact_email`    VARCHAR(100) NOT NULL DEFAULT '' COMMENT '物业联系邮箱',
  ...
  `image_gallery`    JSON         NULL COMMENT '物业图片及启用状态',
  ...
  `amenities`        JSON         NULL COMMENT '分组设施、图标及启用亮点状态',
  `long_stay`        JSON         NULL COMMENT '长住促销与长住权益',
  `hotel_policies`   JSON         NULL COMMENT '预订/入住退房/儿童加床/宠物/物业规则',
  `nearby_attractions` JSON       NULL COMMENT '附近景点:图片、名称、路程与交通方式',
