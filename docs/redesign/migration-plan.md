...
| Phase 1 验证 · 原型对齐整改(2026-08-16) | 已完成·验证通过 | 对照 Figma 原型 stir-long v4.2.1 补齐 5 项差距:① Onboarding 入驻流水线(`database/merchant/09-merchant-application.sql` 5 表 + 9 KYC 模板种子;`OnboardingController` 12 接口 `/api/v1/admin/merchant/onboarding/*`;前端 `views/merchant/onboarding/index.vue` 列表+7 分区抽屉);② Access Code 凭证(approve 生成 `MTRP-{业态}-{6位}` 写入 merchant_info.access_code + 凭证卡/渠道/Regenerate);③ 9 项预置驳回原因(reasonCode 1-9 必填);④ 文档重交版本(`merchant_verify_document_revision` + Original vs Resubmitted 对比);⑤ 最终决策门禁(存在未核验文档拒绝 approve)。菜单种子 205 Onboarding + 6 按钮 + 20105 regencode。验证:db-apply 落库无误、接口全流程冒烟全过、`check.ps1` 四步全绿。边界:merchant-web 商户端 KYC 提交入口后续接;Send KYC/Reminder 仅写审计时间线 |
