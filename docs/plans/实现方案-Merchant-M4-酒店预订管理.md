...
> 状态：2026-09-01 的原 M4 阶段 0～6 历史交付保留；2026-09-18 按 PRD v1.0.3 与 Figma 详情节点 `1289:24340`、列表节点 `1289:16725` 重新复核并完成阶段 0～6 代码整改、自动化回归和真实登录态视觉验收。物业级 No-show 可配置模型仍待产品决策。
> 更新时间：2026-09-18。
> 需求基线：仓库外同级项目目录 `../../documents/PRD/mTrip_Merchant App PRD_v1.0.3_中文版.md` 的“模块 4 —— 预订管理”、场景 3 及预订管理验收标准。
> UI 基线：[Figma `mTrip_Merchant` 详情节点 `1289:24340`](https://www.figma.com/design/fsK2rrl2sadcowrxspvGV8/mTrip_Merchant?node-id=1289-24340&m=dev)与[列表节点 `1289:16725`](https://www.figma.com/design/fsK2rrl2sadcowrxspvGV8/mTrip_Merchant?node-id=1289-16725&m=dev)。
> 说明：2026-09-01 基于 `big-plank-58319748.figma.site` 的历史验收记录保留，本轮以上述 Figma 设计文件节点为当前基线，不复用旧截图代替本轮验收。

---

## 0. 2026-09-18 PRD v1.0.3 / Figma 整改记录

### 0.1 范围与边界

- 范围限于 `merchant-web` 预订管理及其已有后端链路；未扩展 `merchant-app` / `client-app` 页面，未发送真实支付、短信、邮件或 PMS 请求。
- All Properties 请求省略 `X-Mtrip-Property-Id`；选中物业时仅发送正整数，服务端授权物业集合是最终边界。
- 数据库迁移 `V20260918010000__booking-management-remediation.sql` 已应用并登记为 21/21；后续不得修改该已执行迁移，新字段必须新建更高版本。

### 0.2 分阶段执行与自审

| 阶段 | 完成内容 | 自审结论 |
|---|---|---|
| 0 | 建立一次性隔离库专项测试，覆盖迁移幂等、物业范围和通知范围 | 不读写真实业务行，脚本结束自动删除一次性数据库 |
| 1 | 修正取消政策可退上限、重复退款上限和取消/退款库存回补；旧酒店核销入口统一进入预订生命周期 | 政策不可退、部分退款、核销/撤销及管理端取消回归通过 |
| 2 | 通知深链改为数字订单 ID，新增 `property_id` 范围，C 端取消/退款申请通知商户 | 员工、物业账号只能读已授权物业通知，商户级通知保留 |
| 3 | 显式支持 Pay at Hotel，新增 `Mark as Paid` 动作和 `mch:order:mark-paid` 权限 | 只允许到店付、Confirmed/Checked-in 且 Pending/Failed 支付状态；幂等且不伪造第三方流水 |
| 4 | 按 Figma 补齐总额、支付倒计时、Workflow、终态告警、支付方式、Mark as Paid 与 No-show 禁用态，删除重复 Pending Check-in 页签；列表按 `1289:16725` 收敛为单搜索栏、胶囊页签、七列主表格和卡片内分页，高级条件收入筛选弹层 | merchant-web 类型检查/生产构建通过；真实登录态已验收 1440×900、1366×768、393×852、列表筛选、详情面板与入住弹窗，并修复实测发现的支付时间线翻译缺口 |
| 5 | `/order` 支持 All Properties 聚合与页内酒店筛选；选定物业后筛选锁定，切换时清空旧房型和详情 | 覆盖全物业聚合、单物业 Header、跨物业详情 404、伪造 query 不扩权及 Header 格式/授权契约 |
| 6 | No-show 新订单冻结站点 IANA 时区、入住日 `23:59:59` 和首晚房费策略；历史订单回退站点时区，API 返回带偏移的 ISO 截止时间，前端按绝对时间判断 | 按请求级 `site_id` 缓存消除历史订单列表 N+1；专项测试、全量 PHP lint、shared、两个 Web 构建、client 类型检查与差异检查通过 |

### 0.3 登录态验收结果与待决策项

1. 已使用真实 Merchant Web 登录会话完成 1440×900、1366×768 默认列表、约 430px 详情面板、入住操作弹窗及 iPhone 16 393×852 窄屏检查。All Properties 酒店筛选与 Hotel 列正常，页签无重复 Pending Check-in，No-show 提前操作为禁用态；详情面板在 1366px 及窄屏转为全宽下方堆叠。
2. 视觉验收发现并修复时间线 `payment_success` / `payment_failed` 中英文案缺口；重新打开真实订单详情后显示“支付成功”，i18n 警告消失。
3. 已在当前登录态下向 `POST /api/v1/merchant/order/mark-paid` 发送不存在订单 ID 的无副作用探测，返回 HTTP `404` / 业务码 `40401` “预订不存在”，而非 `401`，证明鉴权、路由、权限与业务查找链路已到达。当前 6 笔真实订单均已支付，未对它们执行成功写入；成功路径由隔离契约覆盖，若需登录态成功写入还需专用 Pay at Hotel 测试订单。
4. PRD 没有规定物业级 No-show 配置字段形态。在产品确认“截止时间、首期费用策略、配置入口”前，不新增物业字段、迁移或配置 UI。
...
- [x] 1440×900 与 1366×768 登录态截图对比完成。
