...
## ★ 2026-08-28：S4本地提交授权（最新Git安排）

用户明确授权本次S4本地代码提交，不推送。范围为S4交付58文件及看板修复新增3文件，共61个文件；原ReviewController、start/stop及两份PRD排除且哈希未变。标题为“feat(merchant): 完成 M12 S4 账号安全与模拟登录并修复商户看板”，实际哈希查询Git日志及提交回执。下文“未提交”和HEAD=232fd3e均为历史交付快照；本次授权不延续至S5。最近验证为314项集成、58单测/858断言及双端构建通过；扫码、完整UI等未验边界不变。
