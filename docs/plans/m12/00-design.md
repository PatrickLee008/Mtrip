...
| D3 业务类型 | 首版酒店和餐厅；已有门票业务保留，不重新定义门票枚举或重做门票功能。 |
...
- 待支付订单是否可在停用后首次付款属于新发现的边界，见第12节G3；未确认前不擅自改变支付和库存释放策略。
...
下一步：用户确认上述技术设计和边界后，按01-tasks-and-tests.md开展第一批。未确认前不执行迁移、不修改业务代码、不部署。
