  ...
  <PageContainer>
    <a-spin :spinning="loading">
      <template v-if="detail">
        <div class="detail-head"><div><a-button type="text" @click="router.push('/rooms')"><LeftOutlined />{{ t('rooms.title') }}</a-button><h1>{{ detail.room.room_name }}</h1><p>{{ detail.room.property_name }} · {{ detail.room.room_code || '-' }}</p></div><a-space><a-button v-if="detail.latestRevision?.status === 1" v-perm="'mch:rooms:edit'" @click="withdraw">{{ t('rooms.actions.withdraw') }}</a-button><a-button v-if="detail.latestRevision?.status !== 1" v-perm="'mch:rooms:edit'" type="primary" @click="router.push(`/rooms/${detail.room.id}/edit`)"><EditOutlined />{{ t('common.edit') }}</a-button></a-space></div>
