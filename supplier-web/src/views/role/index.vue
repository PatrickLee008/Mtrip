...
/** 角色管理:CRUD / 菜单授权(供应商端全部菜单树) */
const { t } = useI18n();

const loading = ref(false);
const list = ref<SupplierRole[]>([]);
...
function openEdit(row: SupplierRole): void {
...
async function removeRole(row: SupplierRole): Promise<void> {
...
const permTarget = ref<SupplierRole | null>(null);
...
async function openPerms(row: SupplierRole): Promise<void> {
...
<template>
  <PageContainer>
    <a-card :bordered="false" class="mtrip-card-shadow">
      ...
      <template #extra>
        <a-button v-perm="'sup:role:add'" type="primary" @click="openCreate">
      ...
      >
        <template #bodyCell="{ column, record }">
          ...
          <template v-else-if="column.key === 'action'">
            <a-space :size="0" wrap>
              <a-button v-perm="'sup:role:assign'" type="link" size="small" @click="openPerms(record as SupplierRole)">{{ t('role.assignMenu') }}</a-button>
              <a-button v-perm="'sup:role:edit'" type="link" size="small" :disabled="record.isBuiltin" @click="openEdit(record as SupplierRole)">{{ t('common.edit') }}</a-button>
              <a-popconfirm v-if="!record.isBuiltin" :title="t('common.deleteConfirm')" @confirm="removeRole(record as SupplierRole)">
                <a-button v-perm="'sup:role:delete'" type="link" size="small" danger>{{ t('common.delete') }}</a-button>
              </a-popconfirm>
            </a-space>
          </template>
        </template>
