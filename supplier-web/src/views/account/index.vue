...
import { apiAccountRoles, apiRoleGrant, apiRoleList, type SupplierRole } from '@/api/role';
...
const roleOptions = ref<SupplierRole[]>([]);
...
<template>
  <PageContainer>
    ...
    <a-card :bordered="false" class="mtrip-card-shadow">
      ...
      <template #extra>
        <a-button v-perm="'sup:account:add'" type="primary" @click="openCreate">
      ...
      >
        <template #bodyCell="{ column, record }">
          ...
          <template v-else-if="column.key === 'action'">
            <a-space :size="0" wrap>
              <a-button v-perm="'sup:account:edit'" type="link" size="small" :disabled="record.isOwner" @click="openEdit(record)">{{ t('common.edit') }}</a-button>
              <a-button v-perm="'sup:account:reset-pwd'" type="link" size="small" @click="openResetPwd(record)">{{ t('account.resetPwd') }}</a-button>
              <a-button v-if="!record.isOwner" v-perm="'sup:role:grant'" type="link" size="small" @click="openGrant(record)">{{ t('account.assignRole') }}</a-button>
              <a-popconfirm
                v-if="!record.isOwner"
                :title="record.status === 1 ? t('common.disable') + '?' : t('common.enable') + '?'"
                @confirm="toggleStatus(record)"
              >
                <a-button v-perm="'sup:account:status'" type="link" size="small">
