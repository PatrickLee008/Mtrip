 ...
 *
 * **静态页**:二维码是设计稿导出的静态图;下载凭证 / 查看订单 / 探索玩乐走 comingSoon,
 * 「Back to Home」回到底部 Tab 首页。
 */
  ...
  const rows = [
    {
      ...
      value: formatWeekdayDateYear(BOOKING_DEMO.checkIn, i18n.language),
    },
    {
      key: 'checkOut',
      label: t('hotels.booking.success.checkOut'),
      value: formatWeekdayDateYear(BOOKING_DEMO.checkOut, i18n.language),
    },
    {
      key: 'guests',
      label: t('hotels.booking.success.guests'),
      value: t('hotels.booking.success.guestsValue', {
        adults: BOOKING_DEMO.adults,
        rooms: BOOKING_DEMO.rooms,
      }),
    },
    {
      key: 'paid',
      label: t('hotels.booking.success.totalPaid'),
      value: `${formatAmount(BOOKING_DEMO.paidTotal, currency)} ${currency}`,
      accent: true,
    },
  ];
      ...
      >
        <View style={styles.header}>
          ...
          <Text style={styles.reference}>
            ...
            <Text style={styles.referenceId}>{BOOKING_DEMO.referenceId}</Text>
          </Text>
        </View>
        ...
        <View style={styles.voucherCard}>
          ...
          <View style={styles.voucherText}>
            ...
            <Text style={styles.voucherHotel}>
              {t(`hotels.results.demo.${BOOKING_DEMO.hotelKey}.name`)}
            </Text>
            <View style={styles.voucherAddress}>
              <HomeIcon name="locationOutline" width={9.333} height={11.667} color={colors.textSoft} />
              <Text style={styles.voucherAddressText}>
                {t(`hotels.results.demo.${BOOKING_DEMO.hotelKey}.address`)}
              </Text>
            </View>
        ...
        <View style={styles.upsell}>
          <Text style={styles.upsellText}>
            {t('hotels.booking.success.upsell', {
              city: t(`hotels.results.demo.${BOOKING_DEMO.hotelKey}.address`).split(',').pop()?.trim(),
            })}
          </Text>
          <Pressable
...
const styles = StyleSheet.create({
  ...
  qrInner: { flex: 1, width: '100%', borderRadius: 4, overflow: 'hidden', backgroundColor: '#E5EEFF' },
